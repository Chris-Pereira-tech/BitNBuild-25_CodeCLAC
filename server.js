...
app.post('/scrape-recipe', async (req, res) => {
    ...
    try {
        ...
        // **New Approach: Prioritize JSON-LD structured data**
        $('script[type="application/ld+json"]').each((i, el) => {
            try {
                const json = JSON.parse($(el).html());
                const graph = json['@graph'] || [json]; // Handle both single objects and graph arrays

                for (const item of graph) {
                    if (item['@type'] === 'Recipe' || (Array.isArray(item['@type']) && item['@type'].includes('Recipe'))) {
                        if (item.recipeIngredient && Array.isArray(item.recipeIngredient)) {
                            ingredients = item.recipeIngredient;
                            return false; // Exit the loop once ingredients are found
                        }
                    }
                }
            } catch (e) {
        ...
        // **Fallback Approach: Use AI to parse raw text**
        ...
        // Using manual axios call to ensure v1beta is used with the 2.5 model
...
app.post('/get-recipe', async (req, res) => {
    ...
    try {
        const modelName = 'gemini-2.5-flash-preview-05-20';
        const apiKey = process.env.GEMINI_API_KEY;
        // Using the v1beta endpoint as it's more likely to support advanced JSON formatting
        const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${apiKey}`;
...
`;
        ...
        if (time && time !== 'Any') {
            prompt += `- Max Cooking Time: ${time}\n`;
        }
        if (diet && diet !== 'None') {
            prompt += `- Dietary Preference: ${diet}\n`;
        }
        if (allergies) {
            prompt += `- Allergies to avoid: ${allergies}.\n`;
        }
        if (cookingStyle) {
            prompt += `- The recipe should be in the style of: ${cookingStyle}.\n`;
        }
        ...
        // **THIS IS THE FIX**
        // This regex correctly removes the ```json and ``` markers.
        ...
        if (!recipeData || !nutritionData) {
            throw new Error("AI response was missing structured recipe or nutrition data.");
        }

        const recipesCollection = admin.firestore().collection('recipes');
        const docRef = await recipesCollection.add({
            ingredients: ingredients,
            recipe: recipeData, // Save the recipe object
